...
def signup_view(request):
    if request.method == 'POST':
        ...
        if CustomUser.objects.filter(phone_number = request.POST['phone_number']).exists(): # 전화번호 중복 체크
        ...
        if new_signup_form.is_valid():
            ...
            elif request.POST['password1'] != request.POST['password2']: # 비밀번호와 비밀번호 확인이 다를 때
                messages.info(request, '비밀번호와 비밀번호 확인이 일치하지 않습니다!')
                return redirect ('urlnamesignup')

            elif len(request.POST['password1']) < 8 : # 비밀버호가 8자리 미만일 때
            ...
            elif CustomUser.objects.filter(student_id = request.POST['student_id']).exists(): # 학번 중복 체크
